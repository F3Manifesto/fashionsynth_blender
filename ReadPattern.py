...
class ReadPattern(bpy.types.Panel):
...
class PatternStitch(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for vert in range(-1,len(obj_vertices)-2):
...
def register():
    bpy.utils.register_class(ReadPattern)
    bpy.utils.register_class(PatternStitch)
    
def unregister():
    bpy.utils.unregister_class(ReadPattern)
    bpy.utils.unregister_class(PatternStitch)
